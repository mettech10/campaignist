...
Slideshow is the cheap volume format (Fastlane-shaped). Hook+demo remixes an owned demo clip after a text hook (`POST /api/campaigns/<id>/remix/hook-demo`). Meme: `POST /api/campaigns/<id>/remix/meme`. fal stays gated to product UGC + motion-design.
